...
function find_all_in_cache_path(mod::Symbol)
    ...
    for prefix in Base.LOAD_CACHE_PATH
...
precompilableerror(ex::Base.WrappedException, c) = precompilableerror(ex.error, c)
...
function __precompile__(isprecompilable::Bool=true)
    if (myid() == 1 &&
        Base.JLOptions().use_compilecache != 0 &&
...
function require_modname(name::AbstractString)
    ...
    if Bool(Base.JLOptions().use_compilecache)
        if endswith(name, ".jl")
            tmp = name[1:end-3]
            for prefix in Base.LOAD_CACHE_PATH
...
function compilecache(name::ByteString)
    ...
    cachepath = Base.LOAD_CACHE_PATH[1]
    if !isdir(cachepath)
        mkpath(cachepath)
    end
...
function importModule(modulePath::ASCIIString)
   ...
   prefix = isabspath(modulePath)?"":"$(pwd())/";
   pathArray = split(modulePath,"/");
   if length(pathArray) > 1
      prefix = prefix * reduce((x,y)->"$x$(separator)$y",pathArray[1:end-1]);
   end
   mod = symbol(pathArray[end]);
   if(length(prefix) >= 1)
      push!(LOAD_PATH,prefix);
      require(mod);
      pop!(LOAD_PATH);
   else
      require(mod);
